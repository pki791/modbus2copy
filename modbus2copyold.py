...
def read_registers(client, register_type, address):
    if register_type == 'holding':
        result = client.read_holding_registers(address, 1, unit=Defaults.UnitId)
    elif register_type == 'input':
        result = client.read_input_registers(address, 1, unit=Defaults.UnitId)
    elif register_type == 'coil':
        result = client.read_coils(address, 1, unit=Defaults.UnitId)
    else:
        logging.error(f"Unknown register type: {register_type}")
        return None
    
    if not result.isError():
        logging.info(f"Read value from {register_type} register at address {address}: {result.registers[0]}")
        return result.registers[0]
    else:
        logging.error(f"Failed to read from {register_type} register at address {address}")
        return None

# Function to write registers to the destination Modbus device
def write_registers(client, address, value):
    result = client.write_register(address, value, unit=Defaults.UnitId)
    if not result.isError():
        logging.info(f"Written value {value} to register at address {address}")
    else:
        logging.error(f"Failed to write value {value} to register at address {address}")
...
def main():
    ...
    while True:
        ...
        value = read_registers(source_client, config['register_type'], config['source_address'])
        source_client.close()
        ...
        write_registers(destination_client, config['destination_address'], value)
        ...
        while time.time() % config['interval'] != 0:
            time.sleep(1)
